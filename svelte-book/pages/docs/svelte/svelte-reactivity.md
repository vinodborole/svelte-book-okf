...
```
`class MediaQuery`Creates a media query and provides a `current` property that reflects whether or not it matches.
...
SvelteURLSearchParams,
	`function createSubscriber(start: (update: () => void) => (() => void) | void): () => void`Returns a `subscribe` function that integrates external event-based systems with Svelte's reactivity.
...
<h1>{large.current ? 'large screen' : 'small screen'}</h1>
```
`class MediaQuery extends ReactiveValue<boolean> {…}``constructor(query: string, fallback?: boolean | undefined);`- `query`A media query string
...
export class `class MediaQuery`MediaQuery {
	...
	constructor(`query: any`query) {
		this.#query = `var window: Window & typeof globalThis`The `window`

window.`function matchMedia(query: string): MediaQueryList`The Window interface's `matchMedia()`

matchMedia(`(${`query: any`query})`);
		this.#subscribe = `function createSubscriber(start: (update: () => void) => (() => void) | void): () => void`Returns a `subscribe` function that integrates external event-based systems with Svelte's reactivity.
It's particularly useful for integrating with web APIs like `MediaQuery`, `IntersectionObserver`, or `WebSocket`.
