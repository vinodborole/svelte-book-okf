...
`interface AnimationConfig {…}``delay?: number;``duration?: number;``easing?: (t: number) => number;``css?: (t: number, u: number) => string;``tick?: (t: number, u: number) => void;`
## FlipParams

`interface FlipParams {…}``delay?: number;``duration?: number | ((len: number) => number);``easing?: (t: number) => number;`
