...
$derived(await `function sum(): Promise<number>`sum());
...
$derived(await `function sum(a: Promise<number>, b: number): Promise<number>`sum(`let a: Promise<number>`a, `let b: number`b));
### await_waterfall
...
$derived(await `let bPromise: Promise<number>`bPromise);
### binding_property_non_reactive

`` `%binding%` is binding to a non-reactive property ```` `%binding%` (%location%) is binding to a non-reactive property ``
### console_log_state
